import { PrismaService } from '../../prisma/prisma.service';
import { EquipItemDto } from './dto/equip-item.dto';
export declare class EquipmentService {
    private readonly prisma;
    constructor(prisma: PrismaService);
    findByCharacter(userId: string, characterId: string): Promise<{
        character: {
            id: string;
            name: string;
            class: string;
            level: number;
            xp: number;
            currentHp: number | null;
            maxHp: number;
        };
        equipment: ({
            mainHand: {
                name: string;
                id: string;
                tier: number;
                description: string | null;
                createdAt: Date;
                updatedAt: Date;
                mapId: string | null;
                rarity: import("@prisma/client").$Enums.Rarity;
                slot: import("@prisma/client").$Enums.ItemSlot;
                family: string;
                materialOrigin: import("@prisma/client").$Enums.MaterialOrigin | null;
                requiredGatheringLevel: number;
                gatheringXpPerUnit: number;
                baseGatheringRatePerHour: number | null;
                strengthBonus: number;
                vitalityBonus: number;
                agilityBonus: number;
                precisionBonus: number;
                techniqueBonus: number;
                willpowerBonus: number;
                healFlat: number;
                healPercent: number;
                usableInCombat: boolean;
                usableOutOfCombat: boolean;
                minTier: number | null;
                maxTier: number | null;
                isCraftable: boolean;
                classId: string | null;
            } | null;
            offHand: {
                name: string;
                id: string;
                tier: number;
                description: string | null;
                createdAt: Date;
                updatedAt: Date;
                mapId: string | null;
                rarity: import("@prisma/client").$Enums.Rarity;
                slot: import("@prisma/client").$Enums.ItemSlot;
                family: string;
                materialOrigin: import("@prisma/client").$Enums.MaterialOrigin | null;
                requiredGatheringLevel: number;
                gatheringXpPerUnit: number;
                baseGatheringRatePerHour: number | null;
                strengthBonus: number;
                vitalityBonus: number;
                agilityBonus: number;
                precisionBonus: number;
                techniqueBonus: number;
                willpowerBonus: number;
                healFlat: number;
                healPercent: number;
                usableInCombat: boolean;
                usableOutOfCombat: boolean;
                minTier: number | null;
                maxTier: number | null;
                isCraftable: boolean;
                classId: string | null;
            } | null;
            head: {
                name: string;
                id: string;
                tier: number;
                description: string | null;
                createdAt: Date;
                updatedAt: Date;
                mapId: string | null;
                rarity: import("@prisma/client").$Enums.Rarity;
                slot: import("@prisma/client").$Enums.ItemSlot;
                family: string;
                materialOrigin: import("@prisma/client").$Enums.MaterialOrigin | null;
                requiredGatheringLevel: number;
                gatheringXpPerUnit: number;
                baseGatheringRatePerHour: number | null;
                strengthBonus: number;
                vitalityBonus: number;
                agilityBonus: number;
                precisionBonus: number;
                techniqueBonus: number;
                willpowerBonus: number;
                healFlat: number;
                healPercent: number;
                usableInCombat: boolean;
                usableOutOfCombat: boolean;
                minTier: number | null;
                maxTier: number | null;
                isCraftable: boolean;
                classId: string | null;
            } | null;
            armor: {
                name: string;
                id: string;
                tier: number;
                description: string | null;
                createdAt: Date;
                updatedAt: Date;
                mapId: string | null;
                rarity: import("@prisma/client").$Enums.Rarity;
                slot: import("@prisma/client").$Enums.ItemSlot;
                family: string;
                materialOrigin: import("@prisma/client").$Enums.MaterialOrigin | null;
                requiredGatheringLevel: number;
                gatheringXpPerUnit: number;
                baseGatheringRatePerHour: number | null;
                strengthBonus: number;
                vitalityBonus: number;
                agilityBonus: number;
                precisionBonus: number;
                techniqueBonus: number;
                willpowerBonus: number;
                healFlat: number;
                healPercent: number;
                usableInCombat: boolean;
                usableOutOfCombat: boolean;
                minTier: number | null;
                maxTier: number | null;
                isCraftable: boolean;
                classId: string | null;
            } | null;
            pants: {
                name: string;
                id: string;
                tier: number;
                description: string | null;
                createdAt: Date;
                updatedAt: Date;
                mapId: string | null;
                rarity: import("@prisma/client").$Enums.Rarity;
                slot: import("@prisma/client").$Enums.ItemSlot;
                family: string;
                materialOrigin: import("@prisma/client").$Enums.MaterialOrigin | null;
                requiredGatheringLevel: number;
                gatheringXpPerUnit: number;
                baseGatheringRatePerHour: number | null;
                strengthBonus: number;
                vitalityBonus: number;
                agilityBonus: number;
                precisionBonus: number;
                techniqueBonus: number;
                willpowerBonus: number;
                healFlat: number;
                healPercent: number;
                usableInCombat: boolean;
                usableOutOfCombat: boolean;
                minTier: number | null;
                maxTier: number | null;
                isCraftable: boolean;
                classId: string | null;
            } | null;
            boots: {
                name: string;
                id: string;
                tier: number;
                description: string | null;
                createdAt: Date;
                updatedAt: Date;
                mapId: string | null;
                rarity: import("@prisma/client").$Enums.Rarity;
                slot: import("@prisma/client").$Enums.ItemSlot;
                family: string;
                materialOrigin: import("@prisma/client").$Enums.MaterialOrigin | null;
                requiredGatheringLevel: number;
                gatheringXpPerUnit: number;
                baseGatheringRatePerHour: number | null;
                strengthBonus: number;
                vitalityBonus: number;
                agilityBonus: number;
                precisionBonus: number;
                techniqueBonus: number;
                willpowerBonus: number;
                healFlat: number;
                healPercent: number;
                usableInCombat: boolean;
                usableOutOfCombat: boolean;
                minTier: number | null;
                maxTier: number | null;
                isCraftable: boolean;
                classId: string | null;
            } | null;
        } & {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            characterId: string;
            mainHandId: string | null;
            offHandId: string | null;
            headId: string | null;
            armorId: string | null;
            pantsId: string | null;
            bootsId: string | null;
        }) | null;
        stats: {
            level: number;
            basePrimaryStats: import("../../common/utils/stats.util").PrimaryStats;
            levelBonusStats: import("../../common/utils/stats.util").PrimaryStats;
            equipmentBonusStats: import("../../common/utils/stats.util").PrimaryStats;
            totalPrimaryStats: import("../../common/utils/stats.util").PrimaryStats;
            derivedCombatStats: import("../../common/utils/stats.util").DerivedCombatStats;
        };
    }>;
    equip(userId: string, equipItemDto: EquipItemDto): Promise<{
        message: string;
        equippedItem: {
            id: string;
            name: string;
            slot: import("@prisma/client").$Enums.ItemSlot;
            rarity: import("@prisma/client").$Enums.Rarity;
            tier: number;
            family: string;
            strengthBonus: number;
            vitalityBonus: number;
            agilityBonus: number;
            precisionBonus: number;
            techniqueBonus: number;
            willpowerBonus: number;
        };
        hpChange: {
            oldCurrentHp: number;
            oldMaxHp: number;
            newCurrentHp: number;
            newMaxHp: number;
            maxHpDifference: number;
            currentHpDifference: number;
        };
        equipment: {
            mainHand: {
                name: string;
                id: string;
                tier: number;
                description: string | null;
                createdAt: Date;
                updatedAt: Date;
                mapId: string | null;
                rarity: import("@prisma/client").$Enums.Rarity;
                slot: import("@prisma/client").$Enums.ItemSlot;
                family: string;
                materialOrigin: import("@prisma/client").$Enums.MaterialOrigin | null;
                requiredGatheringLevel: number;
                gatheringXpPerUnit: number;
                baseGatheringRatePerHour: number | null;
                strengthBonus: number;
                vitalityBonus: number;
                agilityBonus: number;
                precisionBonus: number;
                techniqueBonus: number;
                willpowerBonus: number;
                healFlat: number;
                healPercent: number;
                usableInCombat: boolean;
                usableOutOfCombat: boolean;
                minTier: number | null;
                maxTier: number | null;
                isCraftable: boolean;
                classId: string | null;
            } | null;
            offHand: {
                name: string;
                id: string;
                tier: number;
                description: string | null;
                createdAt: Date;
                updatedAt: Date;
                mapId: string | null;
                rarity: import("@prisma/client").$Enums.Rarity;
                slot: import("@prisma/client").$Enums.ItemSlot;
                family: string;
                materialOrigin: import("@prisma/client").$Enums.MaterialOrigin | null;
                requiredGatheringLevel: number;
                gatheringXpPerUnit: number;
                baseGatheringRatePerHour: number | null;
                strengthBonus: number;
                vitalityBonus: number;
                agilityBonus: number;
                precisionBonus: number;
                techniqueBonus: number;
                willpowerBonus: number;
                healFlat: number;
                healPercent: number;
                usableInCombat: boolean;
                usableOutOfCombat: boolean;
                minTier: number | null;
                maxTier: number | null;
                isCraftable: boolean;
                classId: string | null;
            } | null;
            head: {
                name: string;
                id: string;
                tier: number;
                description: string | null;
                createdAt: Date;
                updatedAt: Date;
                mapId: string | null;
                rarity: import("@prisma/client").$Enums.Rarity;
                slot: import("@prisma/client").$Enums.ItemSlot;
                family: string;
                materialOrigin: import("@prisma/client").$Enums.MaterialOrigin | null;
                requiredGatheringLevel: number;
                gatheringXpPerUnit: number;
                baseGatheringRatePerHour: number | null;
                strengthBonus: number;
                vitalityBonus: number;
                agilityBonus: number;
                precisionBonus: number;
                techniqueBonus: number;
                willpowerBonus: number;
                healFlat: number;
                healPercent: number;
                usableInCombat: boolean;
                usableOutOfCombat: boolean;
                minTier: number | null;
                maxTier: number | null;
                isCraftable: boolean;
                classId: string | null;
            } | null;
            armor: {
                name: string;
                id: string;
                tier: number;
                description: string | null;
                createdAt: Date;
                updatedAt: Date;
                mapId: string | null;
                rarity: import("@prisma/client").$Enums.Rarity;
                slot: import("@prisma/client").$Enums.ItemSlot;
                family: string;
                materialOrigin: import("@prisma/client").$Enums.MaterialOrigin | null;
                requiredGatheringLevel: number;
                gatheringXpPerUnit: number;
                baseGatheringRatePerHour: number | null;
                strengthBonus: number;
                vitalityBonus: number;
                agilityBonus: number;
                precisionBonus: number;
                techniqueBonus: number;
                willpowerBonus: number;
                healFlat: number;
                healPercent: number;
                usableInCombat: boolean;
                usableOutOfCombat: boolean;
                minTier: number | null;
                maxTier: number | null;
                isCraftable: boolean;
                classId: string | null;
            } | null;
            pants: {
                name: string;
                id: string;
                tier: number;
                description: string | null;
                createdAt: Date;
                updatedAt: Date;
                mapId: string | null;
                rarity: import("@prisma/client").$Enums.Rarity;
                slot: import("@prisma/client").$Enums.ItemSlot;
                family: string;
                materialOrigin: import("@prisma/client").$Enums.MaterialOrigin | null;
                requiredGatheringLevel: number;
                gatheringXpPerUnit: number;
                baseGatheringRatePerHour: number | null;
                strengthBonus: number;
                vitalityBonus: number;
                agilityBonus: number;
                precisionBonus: number;
                techniqueBonus: number;
                willpowerBonus: number;
                healFlat: number;
                healPercent: number;
                usableInCombat: boolean;
                usableOutOfCombat: boolean;
                minTier: number | null;
                maxTier: number | null;
                isCraftable: boolean;
                classId: string | null;
            } | null;
            boots: {
                name: string;
                id: string;
                tier: number;
                description: string | null;
                createdAt: Date;
                updatedAt: Date;
                mapId: string | null;
                rarity: import("@prisma/client").$Enums.Rarity;
                slot: import("@prisma/client").$Enums.ItemSlot;
                family: string;
                materialOrigin: import("@prisma/client").$Enums.MaterialOrigin | null;
                requiredGatheringLevel: number;
                gatheringXpPerUnit: number;
                baseGatheringRatePerHour: number | null;
                strengthBonus: number;
                vitalityBonus: number;
                agilityBonus: number;
                precisionBonus: number;
                techniqueBonus: number;
                willpowerBonus: number;
                healFlat: number;
                healPercent: number;
                usableInCombat: boolean;
                usableOutOfCombat: boolean;
                minTier: number | null;
                maxTier: number | null;
                isCraftable: boolean;
                classId: string | null;
            } | null;
        } & {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            characterId: string;
            mainHandId: string | null;
            offHandId: string | null;
            headId: string | null;
            armorId: string | null;
            pantsId: string | null;
            bootsId: string | null;
        };
        stats: {
            level: number;
            basePrimaryStats: import("../../common/utils/stats.util").PrimaryStats;
            levelBonusStats: import("../../common/utils/stats.util").PrimaryStats;
            equipmentBonusStats: import("../../common/utils/stats.util").PrimaryStats;
            totalPrimaryStats: import("../../common/utils/stats.util").PrimaryStats;
            derivedCombatStats: import("../../common/utils/stats.util").DerivedCombatStats;
        };
    }>;
    private getEquipmentItems;
    private getEquipmentItemsFromEquipment;
    private buildStatsResponse;
    private clampHp;
    private calculateCurrentHpAfterEquipmentChange;
    private getEquipmentUpdateData;
}
