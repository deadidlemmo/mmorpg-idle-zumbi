...
// A abordagem BALANCED de T1-T5 preserva cerca de 65% do XP/h do
// autocombate representativo. T6-T10 continuam fora do balanceamento de launch.
const tierBalance: Record<
  number,
  {
    costA: number;
    costB: number;
...
const mapIncursions = [
  {
    ...
    minLevel: 1,
    maxLevel: 10,
    names: ['Casas Seladas', 'Porão dos Infectados'],
  },
  {
    ...
    minLevel: 11,
    maxLevel: 20,
    names: ['Galpão do Capataz', 'Oficina Enferrujada'],
  },
  {
    ...
    minLevel: 21,
    maxLevel: 30,
    names: ['Ala de Isolamento', 'Necrotério Lacrado'],
  },
  {
    ...
    minLevel: 31,
    maxLevel: 40,
    names: ['Plataforma Morta', 'Túneis de Embarque'],
  },
  {
    ...
    minLevel: 41,
    maxLevel: 50,
    names: ['Bloco de Contenção', 'Posto de Triagem Selado'],
  },
  {
    ...
    minLevel: 51,
    maxLevel: 60,
    names: ['Sala das Caldeiras', 'Tanques de Cinza'],
  },
  {
    ...
    minLevel: 61,
    maxLevel: 70,
    names: ['Viaduto dos Mortos', 'Comboio Abandonado'],
  },
  {
    ...
    minLevel: 71,
    maxLevel: 80,
    names: ['Laboratório Helix', 'Câmara Experimental'],
  },
  {
    ...
    minLevel: 81,
    maxLevel: 90,
    names: ['Forja dos Corpos', 'Núcleo de Sucata Viva'],
  },
  {
    mapName: 'Marco Zero',
    tier: 10,
    minLevel: 91,
    maxLevel: 100,
    names: ['Cratera Carmesim', 'Santuário da Ruína'],
  },
] as const;
...
function buildLoot(
  tier: number,
  index: number,
): IncursionSeedData['lootTable'] {
  const balance = tierBalance[tier];
  const hasLaunchBalance = isEconomyLaunchTier(tier);
  const lootTable: IncursionSeedData['lootTable'] = [
    ...
    },
    {
      ...
      chance: hasLaunchBalance ? 100 : 25 + tier,
      minQuantity: balance.goldMin,
      maxQuantity: balance.goldMax,
      guaranteed: hasLaunchBalance,
      rarity:
        tier >= 9
          ? Rarity.LEGENDARY
          : tier >= 7
  ...
  if (isEconomyLaunchTier(tier)) {
    ...
    if (!tokenItem) {
      throw new Error(
        `Ficha de incursão canônica não encontrada para T${tier}.`,
      );
    }
