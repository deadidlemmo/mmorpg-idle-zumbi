...
export function EquipmentPage() {
  ...
  const [selectedSlot, setSelectedSlot] =
    useState<EquipmentSlot>('MAIN_HAND');
  ...
  const selectedSlotConfig =
    SLOT_CONFIGS.find((config) => config.slot === selectedSlot) ?? SLOT_CONFIGS[0];
  ...
  const currentClassName =
    character?.class?.name ?? character?.gameClass?.name ?? character?.className;
    ...
    <DashboardLayout character={character} hideHero>
      <main className="equipment-page" aria-label="Equipamentos do personagem">
        ...
        <div className="equipment-overview-grid">
          <section className="equipment-loadout" aria-labelledby="loadout-title">
          ...
          <aside className="equipment-stats" aria-labelledby="equipment-stats-title">
            <div className="equipment-section-heading">
              <div>
                <span>ATRIBUTOS</span>
        ...
        <div className="equipment-management-grid">
                    ...
                    >
                      <span className="equipment-candidate__visual">
                        {imageUrl ? (
                          ...
                          <selectedSlotConfig.Icon size={23} strokeWidth={1.7} />
                        )}
                      </span>
                      <span className="equipment-candidate__copy">
                        <strong>{item.name}</strong>
                        <small>
                          T{item.tier ?? '?'} · {formatInventoryRarity(item.rarity)}
                        </small>
                      </span>
          ...
          <section className="equipment-comparison" aria-labelledby="comparison-title">
            <div className="equipment-section-heading">
              <div>
            ...
            {equippedItem ? (
              ...
              >
                <Unlink size={17} />
                {actionItemId === equippedItem.id ? 'Removendo...' : 'Desequipar'}
              </button>
            ) : null}
          </section>
        </div>
